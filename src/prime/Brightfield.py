  ...
  if img_bw is not None:
    ...
    for img_set in range(len(img_bw)): 
      ...
      if circles_grey is not None and zoom != "ERROR":
        ...
        if zoom == "large":
          accurate_circles = detect.Circle_Finder_Improve(circles_grey,processed_img)
          bead_count = 0 
          nobead_count = 0
          bead_count, nobead_count, circles_with_beads = bc.Bead_Counting(accurate_circles, bead_img_copy)
        ...
        if filter>50 and BAD_IMAGE == False:
          ...
          circles_img = visual.Draw_Circles(processed_img, accurate_circles,circles_with_beads)
          """ 
          st.write("#################################################")
